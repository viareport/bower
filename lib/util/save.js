...
function savePkgs(manager, paths, dev, cb) {
  manager.on('loadJSON', function (newLine) {
    ...
    var names = _.compact(paths.map(function (endpoint) {
      ...
      for(var type in manager.dependencies) {
        for (var name in manager.dependencies[type]) {
          var dep = manager.dependencies[type][name][0];
          if (dep.name === endpoint) {
            validDependencies.push(dep);
            return;
          }
          
          var fetchedEndpoint = dep.readEndpoint();
          if(fetchedEndpoint && fetchedEndpoint.endpoint === endpoint) {
            validDependencies.push(dep);
          }
          return;
        }
    ...
    // Save each of them
    // Only include the root packages
    validDependencies.forEach(function (pkg) {
      addDependency(manager.json, pkg, dev);
    });
...
function addDependency(json, pkg, dev) {
  ...
  json[key][pkg.json.repository.type] = json[key][pkg.json.repository.type] || {};
  json[key][pkg.json.repository.type][pkg.name] = path ? path + (tag ? '#' + tag : '') : tag || 'latest';
}
