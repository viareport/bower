...
Package.prototype.resolveHttp = function () {
  // Ensure that nobody is resolving the same dep at the same time
  // If there is, we wait for the unlock event
  if (this.unitWork.isLocked(this.name)) return this.unitWork.on('unlock', this.waitUnlock);
  ...
  if (this.assetUrl) {
    ...
    this.once('lookupHttp', this.download).lookupHttp();
  }

  return this;
};

Package.prototype.lookupHttp = function () {
  source.lookupHttp({name: this.name, version:this.tag}, function (err, version) {
    if (err) return this.emit('error', err);
    var url = version.url;
    this.version = version.version;
    this.lookedUp = true;
    this.assetUrl = url;
    this.assetType = path.extname(url);
    this.emit('lookupHttp');
  }.bind(this));
};
...
Package.prototype.getType = function () {
  ...
  if (this.json.repository.type === 'git' || this.gitUrl) {
    return 'git';
  }
  if (this.json.repository.type === 'local-repo' || this.gitPath) {
    return 'local-repo';
  }
  if (this.json.repository.type === 'local' || this.path) {
    return 'local';
  }
  if (this.json.repository.type === 'asset' || this.assetUrl) {
    return 'asset';
  }
};
