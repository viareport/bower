...
var http_endpoint = config.http_endpoint + '/packages';
...
exports.lookupHttp = function (pkg, callback) {
  // walk all endpoints to find the first matching component
  var f = function (i) {
    var endpoint = http_endpoint;
    request.get(endpoint + '/' + encodeURIComponent(pkg.name), function (err, response, body) {
      if (err || (response.statusCode !== 200 && response.statusCode !== 404)) {
        return callback(err || new Error(name + ' failed to look up for endpoint: ' + endpoint));
      }
      ...
      if (response && response.statusCode !== 404) {
        ...
        versions.sort(function(o1,o2) {
          return semver.rcompare(o1.version,o2.version);
        });
        if(pkg.version) {
          versions = versions.filter(function(elem) {
            return new RegExp(pkg.version).exec(elem.version);
          },this);
        }
        if(versions.length === 0) {
          callback(new Error('Version not found : ' + pkg.version), body && versions[0]);
        } else {
          callback(err, body && versions[0]);
        }
      } else {
        if (i + 1 < endpoints.length) f(i + 1);
        else return callback(new Error(pkg.name + ' not found'));
      }
    });
  };
  f(0);
...
exports.registerHttp = function (name, url, version, callback) {
  var body = {name: name, url: url, version: version};

  request.post({url: http_endpoint, form: body}, function (err, response) {
    if (err) return callback(err);
