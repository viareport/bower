...
Manager.prototype.resolveFromJson = function () {
  this.once('loadJSON', function () {
    ...
    async.forEach(Object.keys(dependencies['asset']), function (name, next) {
      var endpoint = dependencies['asset'][name];
      var pkg      = new Package(name, endpoint, this);
      var errorNext;

      pkg.root = true;
      this.dependencies['asset'] = this.dependencies['asset'] || [];
      this.dependencies['asset'][name] = this.dependencies['asset'][name] || [];
      this.dependencies['asset'][name].push(pkg);
