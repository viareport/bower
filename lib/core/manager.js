...
Manager.prototype.resolveHttp = function () {
  var resolved = function () {
    // If there is errors, report them
    if (this.errors.length) return this.reportErrors();
    // Otherwise all is fine, so we install
  ...
  if (this.endpoints.length) {
    ...
    //       calling resolved() (slower)
    //       This will solve issue #200
    this.once('resolveHttpEndpoints', resolved).resolveHttpEndpoints();
  } else {
    this.once('resolveFromJson', resolved).resolveFromJsonHttp();
  }
...
Manager.prototype.resolveLocal = function () {
  ...
  glob('./' + config.directory + '/*', function (err, dirs) {
    ...
    dirs.forEach(function (dir) {
      ...
      this.dependencies[type][name].push(pkg);
...
Manager.prototype.resolveHttpEndpoints = function () {
  var type = 'asset';
  ...
  async.forEach(this.endpoints, function (endpoint, next) {
    ...
    this.dependencies[type][endpoint] = this.dependencies[type][endpoint] || [];
    this.dependencies[type][endpoint].push(pkg);
  ...
  }.bind(this), this.emit.bind(this, 'resolveHttpEndpoints'));
...
Manager.prototype.resolveFromJson = function () {
  this.once('loadJSON', function () {
    ...
    async.forEach(Object.keys(dependencies), function (type) {
      if(type !== 'asset') {
        async.forEach(Object.keys(dependencies[type]), function (name, next) {
          var endpoint = dependencies[type][name];
          var pkg      = new Package(name, endpoint, this);
          var errorNext;
    
          pkg.root = true;
          this.dependencies[type] = this.dependencies[type] || [];
          this.dependencies[type][name] = this.dependencies[type][name] || [];
          this.dependencies[type][name].push(pkg);
...
Manager.prototype.resolveFromJsonHttp = function () {
  this.once('loadJSON', function () {
    var dependencies = this.json.dependencies || {};
    ...
    if(!dependencies[type]) {
      this.emit('error',new Error('no http dependencies'));
      return this;
    }
    async.forEach(Object.keys(dependencies[type]), function (name, next) {
      var endpoint = dependencies[type][name];
...
Manager.prototype.getDeepDependencies = function () {
  ...
  for (var type in this.dependencies) {
    if(type !== 'asset') {
      for (var name in this.dependencies[type]) {
        this.dependencies[type][name].forEach(function (pkg) {
          result[type] = result[type] || [];
          result[type][pkg.name] = result[type][pkg.name] || [];
          result[type][pkg.name].push(pkg);
...
Manager.prototype.prune = function () {
  ...
  if (result.conflicted) {
    for (var type in result.conflicted) {
      for (var name in result.conflicted[type]) {
        this.reportConflicts(name, result.conflicted[type][name]);
      }
    }
  ...
  if (result.forceblyResolved) {
    for (var type in result.forceblyResolved) {
      for (var name in result.forceblyResolved[type]) {
        this.reportForceblyResolved(name, result.forceblyResolved[type][name]);
        this.dependencies[type][name] = result.forceblyResolved[type][name];
        this.dependencies[type][name][0].root = true;
      }
    }
  }
...
Manager.prototype.install = function () {
  async.forEach(Object.keys(this.dependencies), function (type) {
    async.forEach(Object.keys(this.dependencies[type]), function (name, next) {
      var pkg = this.dependencies[type][name][0];
...
Manager.prototype.getDependencyList = function () {
  ...
  Object.keys(this.dependencies).forEach(function (type) {
    Object.keys(this.dependencies[type]).forEach(function (name) {
      packages[name] = this.dependencies[type][name][0];
    }.bind(this));
  }.bind(this));
